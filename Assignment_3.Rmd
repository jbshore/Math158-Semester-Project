...
We did log on sqft_lot because all of them are bunched up around smaller values. We did log on price to get rid of heteroskedasticity. This is more apparent below when we look at residuals.

In order to figure out which variables are interacting, we compare the non-interacting model with the interacting model, and since log(sqft_lot) and long are insignificant so the interaction terms that we keep are sqft_living:condition, sqft_living:view, and view:lat.
