...
```
$R^2$ value is 0.456. That means 45.6% of the variance in $Log(price)$ is explained by the linear relationship between $log(sqft_living)$ and $log(price)$. In the residual plot, we see constant variability and it's a good fit. 
...
The result is that there is a positive relationship between sqft_living and price, meaning in general, the bigger the house, the more expensive it is. For the most part, the data were what we expected, as we can see from the scatter plot. What surprised us was the residual plot before the transformation, how it